...
export default function FeaturedMatchCard({
  ...
  return (
    <div className={`bg-slate-900/40 rounded-2xl border p-4 flex flex-col justify-between transition-all duration-300 ease-out hover:-translate-y-1 hover:scale-[1.02] hover:shadow-[0_20px_25px_-5px_rgba(0,0,0,0.5)] group relative text-right ${
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-955/80 backdrop-blur-sm p-4 text-slate-200" dir="rtl">
          <div className="w-full max-w-md bg-slate-900 rounded-3xl border border-emerald-500/30 overflow-hidden shadow-2xl relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-5 border-b border-slate-800/80 bg-slate-950 flex justify-between items-center text-right">
              <div>
                ...
                <p className="text-xs text-slate-400">{STAGE_TRANSLATIONS[match.stage] || match.stage} · ورزشگاه {match.stadium}</p>
              </div>
              <button 
                type="button"
                onClick={() => setIsModalOpen(false)}
                className="text-slate-400 hover:text-white px-2.5 py-1 rounded-lg hover:bg-slate-800 font-bold transition-all font-sans"
              >
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleModalSave} className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-7 items-center justify-center p-3 sm:p-4 bg-slate-950/60 rounded-2xl border border-slate-800/80">
                ...
                <div className="col-span-3 text-center space-y-2">
                  <div className="flex justify-center select-none">
                    <FlagIcon teamIdOrCode={match.homeTeamId} className="h-7 w-10" />
                  </div>
                  <span className="font-bold text-slate-100 block text-xs truncate max-w-full">{homeName}</span>
                  <div className="inline-flex items-center">
                    <button 
                      type="button" 
                      onClick={() => setHomeInput(h => Math.max(0, h - 1))}
                      className="px-2.5 py-1 bg-slate-805 hover:bg-slate-700 font-black rounded-lg text-slate-100 hover:text-emerald-400 transition-all font-mono"
                    >
                      -
                    </button>
                    <input 
                      ...
                      className="w-12 bg-slate-950 border border-slate-800 text-center text-lg font-black font-mono text-white rounded-lg focus:outline-none focus:border-emerald-500 py-1 mx-1.5"
                    />
                    <button 
                      type="button" 
                ...
                <div className="col-span-3 text-center space-y-2">
                  <div className="flex justify-center select-none">
                    <FlagIcon teamIdOrCode={match.awayTeamId} className="h-7 w-10" />
                  </div>
                  <span className="font-bold text-slate-100 block text-xs truncate max-w-full">{awayName}</span>
                  <div className="inline-flex items-center">
                    <button 
                      type="button" 
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="flex-1 px-4 py-2.5 bg-slate-850 hover:bg-slate-800 text-slate-300 text-sm font-semibold rounded-xl border border-slate-855 transition-colors font-sans"
                >
                  انصراف
                </button>
                <button
